...
class Player
{
    ...
    public function Run(): Response|array|string
    {
        ...
        if (!$this->session->GetCheck()) {
            if ($this->session->reason == 'PLEASE_LOGIN') {
                $module = 'welcome';
                $action = 'login';
            ...
            if (!empty($this->app->Secure->GET['module']) && !empty($this->app->Secure->GET['action']) &&
                in_array($this->app->Secure->GET['module'], ['backup', 'systemtemplates'],
                ) && $this->app->Secure->GET['action'] === 'readstatus') {
                $sessionFile = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'backup' . DIRECTORY_SEPARATOR . 'session.txt';
                if (file_exists($sessionFile) && ((string)$bckSession = file_get_contents($sessionFile)) && trim(
                        $bckSession,
                    ) === session_id()) {
        ...
        if (file_exists(dirname(__DIR__) . '/www/pages/' . $module . '.php')) {
            if (file_exists(dirname(__DIR__) . '/www/pages/' . $module . '_custom.php')) {
                ...
                if (method_exists(
                        $constr,
                        'AllowedVersion',
                    ) && isset($this->app->User) && $this->app->User && method_exists(
                        $this->app->User,
                        'GetType',
                    ...
                    if ($r2->isStatic()) {
                        ...
                        if ((isset($allowed['max']) && ((float)$allowed['max'] < (float)$version_revision))
                            ...
                            (isset($allowed['versionen']) && (
                                    (is_array($allowed['versionen']) && !in_array(
                                            $version_revision,
                                            $allowed['versionen'],
                                        ))
        ...
        if (class_exists('Xentral\Modules\SystemNotification\Bootstrap', true)) {
            ...
            $stylesheets = forward_static_call(
                ['Xentral\\Modules\\SystemNotification\\Bootstrap', 'registerStylesheets'],
            );
        ...
        if (!$permission) {
            if ($this->app->User->GetID() <= 0) {
                ...
                return new Response(
                    str_replace(
                        'BACK',
                        "index.php?module=welcome&action=login",
                        $this->app->Tpl->FinalParse("permissiondenied.tpl"),
                    ),
                );
            } else {
